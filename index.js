...
function getRandomFlavors(originalFlavors, newFlavors, seasonalFlavors, regionalFlavors){
  ...
  let randomFlavors = []
  function selection(allFlavors) {
    return Math.floor(Math.random() * (allFlavors.length - 1));
  }
  for (; randomFlavors.length < 31; ) {
    let curSelection = allFlavors[selection(allFlavors)];
    if(!randomFlavors.includes(curSelection)) {
        randomFlavors.push(curSelection)
    } else { continue }
  }
